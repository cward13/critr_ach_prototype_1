...
def main(settings):
	...
	print 'start'
	try:
#	        Txport = serial.Serial('/dev/ttyAMA0', baudrate = baudRate, timeout = 0)
#	        Txport.open();
...
#					screen.addstr(1,0,'up')
					user_input = "1 "
				elif char == curses.KEY_DOWN:
#					screen.addstr(1,0,'down')
					user_input = "2 "
				elif char == ord('s') or char == ord(' '):
#					screen.addstr(1,0,'stop')
					user_input = "0 "
#				Txport.flush()
#				screen.addstr(2,0,Txport.read(Txport.inWaiting()))
				ard.write(my_bytes)
				#time.sleep(.1)
